...
Example (OpenEnv sync client):
    >>> env = GymEnv(base_url="http://localhost:8001").sync()
    >>> with env:
    ...     result = env.reset()
    ...     result = env.step({"value": 1})
    ...     print(result.observation)

Example (HTTP client — stateless, useful for health checks):
    >>> client = GymEnvHttpClient("http://localhost:8001")
    >>> obs = client.reset()  # one-shot reset
    >>> client.close()
"""
...
class GymEnvHttpClient:
    """Lightweight synchronous HTTP client for quick scripting against the gym server.

    Uses the standard OpenEnv HTTP endpoints (POST /reset, POST /step, GET /state).
    """
