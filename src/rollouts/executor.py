...
from src.envs.gym_wrapper.client import GymEnvClient
...
def run_episodes(
    ...
    client = GymEnvClient(base_url=env_url)
    results = []
    ...
    for ep in range(num_episodes):
        ...
        for step_num in range(max_steps):
            # Simple random policy for now (will be replaced by agent-generated policies)
            import random
            action = random.randint(0, 1)  # CartPole has 2 discrete actions
